# Code adapted from https://github.com/zhhoper/DPR
...
from model/model import HourglassNet
# other modules
import os
import numpy as np
...
my_network.load_state_dict(torch.load(os.path.join(modelFolder, model)))
my_network.cuda()
my_network.train(False)

lightFolder = 'data/light/'

saveFolder = 'test/result'
if not os.path.exists(saveFolder):
    os.makedirs(saveFolder)

img = cv2.imread(os.path.join('data/test/', input_image)
row, col, _ = img.shape
...
for i in range(7):
    ...
    cv2.imwrite(os.path.join(saveFolder, '{}{:02d}.jpg'.format(f, i)), resultLab)
    #----------------------------------------------